    ...
    <> 
     ...
     <div className="flex flex-col gap-2">
      ...
      {list.map((item, index)=>(
        ...
        <img src={item.image[0]} className='w-20 h-15 object-cover' alt="" />
        <p>{item.name}</p>
            <p>{item.category}</p>
            <p>{currency} {item.price}</p>
            <p onClick={() => removeProduct(item._id)} className='text-right md:text-center cursor-pointer text-sm text-red-500'>Delete</p>
            </div>
          ))}
